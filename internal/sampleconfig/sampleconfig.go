...
// GetConfig returns the current sample config object. This object is a shared singleton. This will panic if there are problems
func GetConfig() (config *Config) {
	once.Do(func() { // <-- atomic, does not allow repeating
		...
		exPath, err = directory.FileWalk(exPath, "sample.conf")
		...
		if exPath != "" {
			log.Infof("sample config path: %s", exPath)
		...
		if err := viper.ReadInConfig(); err != nil {
			if _, ok := err.(viper.ConfigFileNotFoundError); ok {
				log.Debugf("sample.conf config file not found")
			} else {
