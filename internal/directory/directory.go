...
// Init initialises the ros service. Basically copies the required scripts to the users
// home directory. (.sample/scripts)
func Init() error {
	homedir, err := Homedir()
	if err != nil {
		return err
	...
	sampleDir := filepath.Join(homedir, ".sample")
	err = os.RemoveAll(sampleDir)
	if err != nil {
		return err
	}
...
// initfolder initialises the sampledir and copies the asset files to the given path
func initfolder(sampleDir, folder string) error {
	// func initfolder(sampleDir, folder string, logger *adapter.Logger) error {
	// copy the scripts to the ~/.sample directory
	sampleInitFolder := filepath.Join(sampleDir, folder)
	err := os.Mkdir(sampleInitFolder, 0755)
	if err != nil {
		return err
	}
	...
	for _, file := range files {
		...
		scriptPath := filepath.Join(sampleInitFolder, file.Name())
